...
class INET_API Packet : public cPacket, public IPrintableObject
{
  ...
  public:
     ...
     * chunk is automatically marked immutable. The front and back offsets are
     * not affected. The offset must be in the range [0, getTotalLength()].
     */
     ...
     * packet and returns it as a mutable chunk in the
     * current representation. If the length is unspecified, then the length of
     * the result is chosen according to the current representation. The front
     ...
     * chunk in the current representation. The front and back offsets are not
     * affected. The flags parameter is a combination of Chunk::PeekFlag enumeration
     * members.
     */
    ...
    const Ptr<T> removeData(int flags = 0) {
        const auto& oldContent = peekData<T>(flags);
    ...
    const Ptr<T> removeAll(int flags = 0) {
        const auto& oldContent = peekAll<T>(flags);
        const auto result = makeExclusivelyOwnedMutableChunk(oldContent);
        eraseAll();
        return result;
    }
    ...
    const Ptr<T> removeDataAt(b offset, b length = b(-1), int flags = 0) {
        const auto& oldContent = peekDataAt<T>(offset, length, flags);
        const auto result = makeExclusivelyOwnedMutableChunk(oldContent);
        eraseDataAt(offset, length);
        return result;
    }
    ...
    const Ptr<T> removeAt(b offset, b length = b(-1), int flags = 0) {
        const auto& oldContent = peekAt<T>(offset, length, flags);
        const auto result = makeExclusivelyOwnedMutableChunk(oldContent);
        eraseAt(offset, length);
        return result;
    }
    //@}
     ...
     */
    template <typename T>
    const Ptr<T> replaceAt(const Ptr<const Chunk>& replacementChunk, b offset, b length = b(-1), int flags = 0) {
        auto totalLength = getTotalLength();
        CHUNK_CHECK_USAGE(replacementChunk != nullptr, "chunk is nullptr");
        CHUNK_CHECK_USAGE(b(0) <= offset && offset <= totalLength, "offset is out of range");
        CHUNK_CHECK_USAGE(b(-1) <= length && offset + length <= totalLength, "length is invalid");
        constPtrCast<Chunk>(replacementChunk)->markImmutable();
        ...
        CHUNK_CHECK_USAGE(chunkLength == replacementChunk->getChunkLength(), "length is different");
        if (frontLength == b(0) && backLength == b(0))
            content = replacementChunk;
        else {
            const auto& sequenceChunk = makeShared<SequenceChunk>();
            if (frontLength > b(0))
                sequenceChunk->insertAtBack(frontPart);
            sequenceChunk->insertAtBack(replacementChunk);
            if (backLength > b(0))
     ...
     * are not affected. The designated part must be in the range [0, getTotalLength()].
     */
